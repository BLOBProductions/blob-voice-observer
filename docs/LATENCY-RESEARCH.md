...
BLOB Voice Observer is a Windows tool that converts spoken digits ("zero" through "nine") into keyboard input for VALORANT observer camera switching. The tool must feel near-realtime for live esports production.
...
- **Restricted grammar**: only 10 words + `[unk]`, i.e. `["zero","one","two","three","four","five","six","seven","eight","nine","[unk]"]`
...
Vosk's `AcceptWaveform()` only returns `True` when it detects an **endpoint**, i.e. a silence gap after speech. The default endpoint detection requires ~500-800ms of silence before finalizing. This is the main source of latency.
...
- **Heisenbug behavior**: Adding debug `print()` statements changed timing enough to mask bugs, a classic sign of a race between our state tracking and Vosk's internal state.
